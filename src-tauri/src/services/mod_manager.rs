...
impl ModManager {
    ...
    pub async fn search_modrinth(&self, query: &str, game_version: &str, loader: &str) -> Result<Vec<ModInfo>, String> {
        let url = format!(
        ...
        for hit in data.hits {
            // 获取最新版本以获取下载链接
            if let Ok(version) = self.get_latest_modrinth_version(&hit.project_id, game_version, loader).await {
    ...
    async fn get_latest_modrinth_version(&self, project_id: &str, game_version: &str, loader: &str) -> Result<ModrinthVersionFile, String> {
        let url = format!(
